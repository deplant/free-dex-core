...
contract TONTokenWallet is ITONTokenWallet {
    ...
    constructor() public {
        ...
        if (owner_address.value != 0) {
            IRootTokenContract(owner_address).notifyWalletDeployed{value: 0.00001 ton}(root_address);
        }
    }
    ...
    function internalTransfer(
        ...
        if (notify_receiver && receive_callback.value != 0) {
            IRootTokenContract(receive_callback).tokensReceivedCallback{ value: 0, flag: 128 }(
                address(this),
                root_address,
                tokens,
                sender_public_key,
...
/*    function burnByOwner(
        ...
        if (owner_address.value != 0 ) {
            tvm.rawReserve(math.max(target_gas_balance, address(this).balance - msg.value), 2);
            balance -= tokens;
            IRootTokenContract(root_address)
    ...
    onBounce(TvmSlice body) external {
        ...
        } else if (functionId == tvm.functionId(IRootTokenContract.tokensBurned)) {
            balance += body.decode(uint128);
            if (owner_address.value != 0) {
                tvm.rawReserve(math.max(target_gas_balance, address(this).balance - msg.value), 2);
                owner_address.transfer({ value: 0, flag: 128 });
